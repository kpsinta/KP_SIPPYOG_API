...
class LaporanTiketController extends RestController
{
    ...
    {
        $tiket = Tiket::whereDate('waktu_keluar',$waktu_keluar)->where('status_tiket',$status)->withTrashed()->get();
    ...
    public function showByStatusTiketAll_Bulanan($status, $waktu_keluar)
    {
        ...
        $tiket = Tiket::whereMonth('waktu_keluar',$month)->whereYear('waktu_keluar',$year)->where('status_tiket',$status)->withTrashed()->get();
        $response = $this->generateCollection($tiket);
        return $this->sendResponse($response);
     }
     public function showByStatusTiketAll_Tahunan($status, $waktu_keluar)
     {
        $tiket = Tiket::whereYear('waktu_keluar',$waktu_keluar)->where('status_tiket',$status)->withTrashed()->get();
        $response = $this->generateCollection($tiket);
        return $this->sendResponse($response);
     } 
}
